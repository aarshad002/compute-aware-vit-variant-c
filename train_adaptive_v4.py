"""
Train adaptive token-pruning DeiT-tiny v4 on CIFAR-100.

Identical to v3 except pruning happens after block 6 (instead of block 3).
Blocks 1-6 process all 197 tokens; blocks 7-12 process the pruned sequence.

Full objective (same as v3):
    L = L_aux + L_blend + λ_budget · budget_cost − λ_ent · H

Hyperparameters (same as v3):
    lr=1e-4, Adam, batch=32, seed=42, weight_decay=1e-4
    λ_budget=0.5, warmup=5 epochs
    ent_init=0.5, anneal=10 epochs
    tau: 3.0 → 0.5
"""
import argparse
import json
...
from src.models.adaptive_pruned_v4 import AdaptivePrunedDeiTv4, BUDGET_TOKENS
...
def main():
    ...
    parser.add_argument('--out_dir', default='outputs/adaptive_v4')
    ...
    print(f'Prune after block: {model.prune_after_block}', flush=True)
    ...
    metrics = {
        'model': 'deit_tiny_adaptive_v4',
        'prune_after_block': model.prune_after_block,
